...
public class ARFFFile
{
	...
	public boolean read()
	{
		...
		try
		{
					...
					(line = br.readLine()) != null)
			{
				...
				else if (line.charAt(0) == '@')
				{
					...
					else if (declFields.get(0).compareToIgnoreCase("@ATTRIBUTE") == 0)
					{
					...
					else if (declFields.get(0).compareToIgnoreCase("@DATA") == 0)
					{
		...
		catch (IOException e)
		{
			ProblemManager.reportException(e, 
					"Failed to open assignment file '" + path + "'.");
	...
	public boolean onReadEnd()
	{
		...
		else
		{
			...
			concernModel.clearQueuedEvents();
			concernModel.enableNotifications();
			concernModel.modelChanged(ConcernEvent.createAllConcernsChanged());
			...
			String expectedDatatype)
	{
		...
		else if (fields.get(2).compareToIgnoreCase(expectedDatatype) != 0)
		{
			ProblemManager.reportError("Invalid ARFF File",
