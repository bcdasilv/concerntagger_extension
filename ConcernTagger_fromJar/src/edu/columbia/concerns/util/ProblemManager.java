...
public final class ProblemManager
{
	                                   ...
	                                   final boolean limitPopups)
	{
		...
		statuses[0] = new Status(IStatus.ERROR, ConcernTagger.ID_PLUGIN,
				IStatus.OK, msg, e);

		for (int i = 0; i < stackElements.length; i++)
		{
			statuses[i + 1] = new Status(IStatus.ERROR,
					ConcernTagger.ID_PLUGIN, IStatus.OK, "     "
		...
		MultiStatus multiStatus = new MultiStatus(ConcernTagger.ID_PLUGIN,
	                             ...
	                             final boolean limitPopups)
	{
		...
		if (details != null)
		{
			IStatus[] statuses = new IStatus[] {
				new Status(severity, ConcernTagger.ID_PLUGIN, details)
			};

			status = new MultiStatus(ConcernTagger.ID_PLUGIN,
					IStatus.OK, statuses, msg, null);
		}
		else
		{
			status = new Status(severity, ConcernTagger.ID_PLUGIN, msg);
		}
	                              ...
	                              final boolean limitPopups)
	{
		...
		ConcernTagger.singleton().getLog().log(status);
